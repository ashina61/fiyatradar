...
/// store_places pending kuyruğu — community önerilen marketleri admin
/// onayına alır. Onay → `status='verified'`, ret → `isActive=false`.
class AdminStorePlacesPendingScreen extends StatelessWidget {
  const AdminStorePlacesPendingScreen({super.key});
          ...
          children: [
            ...
            Expanded(
              child: StreamBuilder<List<Map<String, dynamic>>>(
                stream: state.watchAdminPendingStorePlaces(),
                builder: (context, snap) {
                  if (snap.connectionState == ConnectionState.waiting &&
                      !snap.hasData) {
                    return const Center(
