                ...
                child: Column(
                  children: [
                    _switchTile(
                      ...
                      onChanged: (val) => ref.read(profileProvider.notifier).updateProfile(user.copyWith(priceAlarm: val)),
                    ),
                    _switchTile(
                      title: 'Kampanya Bildirimleri',
                      subtitle: 'A101, BİM vb. haftalık indirimleri.',
                      value: user.campaignNotification,
                      onChanged: (val) => ref.read(profileProvider.notifier).updateProfile(user.copyWith(campaignNotification: val)),
                    ),
                    _switchTile(
                      title: 'Rozet ve Puan Bildirimleri',
                      subtitle: 'Liderlik tablosunda yükseldiğinde uyar.',
                      value: user.badgeNotification,
                      onChanged: (val) => ref.read(profileProvider.notifier).updateProfile(user.copyWith(badgeNotification: val)),
                    ),
                  ],
                ),
              ),
