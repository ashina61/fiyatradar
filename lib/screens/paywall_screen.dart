...
class PaywallScreen extends StatefulWidget {
  const PaywallScreen({super.key, this.service});

  /// Test seam. Production passes nothing and the screen uses the
  /// `PremiumService` singleton; widget tests inject a fake.
...
class _PaywallScreenState extends State<PaywallScreen> {
  ...
  PremiumService get _svc => widget.service ?? PremiumService.instance;
  ...
  void initState() {
    ...
    if (!_svc.productsQueried && !_svc.loadingProducts) {
      unawaited(_svc.reloadProducts());
    }
  }
  ...
  Future<void> _restore() async {
    ...
    // aslında erişilebilir, o yüzden CTA gibi burada da yalnızca gerçekten
    // hiçbir şey yokken — `!available` VE ürün listesi boşken — bail et.
