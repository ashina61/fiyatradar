/// Authoritative whitelist of Turkish provinces (iller) and their districts
/// (ilçeler). Keeping this list canonical prevents free-text drift in user
/// region settings and store metadata, e.g. "İstanbul" / "istanbul" /
/// "Istanbul" being treated as different cities.
///
/// All entries use the canonical Turkish spelling with proper diacritics
/// (İ, ı, ş, ç, ğ, ö, ü). The keys of [districtsByCity] are the canonical
/// city names; treat all reads through [TurkeyLocations] which normalizes
/// input.
library;
...
class TurkeyLocations {
  ...
  /// Canonical sorted list of all 81 Turkish provinces.
  ...
  /// Districts (ilçeler) grouped by their parent province. The list is
  /// intentionally exhaustive for the 81 cities so the UI doesn't need to
  /// fall back to free text.
  static const Map<String, List<String>> districtsByCity = <String, List<String>>{
  ...
  /// Returns the canonical district list for [city] (already trimmed); empty
  /// when the city isn't in the whitelist.
  static List<String> districtsOf(String? city) {
    final canonical = canonicalCity(city);
    if (canonical == null) return const <String>[];
    return districtsByCity[canonical] ?? const <String>[];
  }

  /// Resolves an arbitrary user / geocoder string to its canonical city
  /// spelling, or null when the input doesn't match any province.
  static String? canonicalCity(String? raw) {
    final key = _normalize(raw);
    if (key.isEmpty) return null;
    for (final c in cities) {
      if (_normalize(c) == key) return c;
    }
    return null;
  }

  /// Resolves an arbitrary district string to its canonical spelling under
  /// [city]. Returns null when either the city or district isn't whitelisted.
  static String? canonicalDistrict(String? rawCity, String? rawDistrict) {
    final city = canonicalCity(rawCity);
    if (city == null) return null;
    final list = districtsByCity[city] ?? const <String>[];
    final key = _normalize(rawDistrict);
    if (key.isEmpty) return null;
    for (final d in list) {
      if (_normalize(d) == key) return d;
    }
    return null;
  }
